...
const QuestionEditor: React.FC<QuestionEditorProps> = ({ question, allQuestions, updateQuestion, deleteQuestion }) => {
  ...
  // Using useCallback helps prevent unnecessary re-renders of child components
            ...
            {(question.details.type === 'short-text' || question.details.type === 'long-text') && (
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <input type="number" value={question.details.maxLength || ''} onChange={e => handleDetailChange('maxLength', e.target.value ? parseInt(e.target.value) : undefined)} className="w-24 p-1 border rounded-md text-sm" />
                </div>
              </div>
            )}
            {question.details.type === 'numeric' && (
              <div>
                ...
                <div className="flex items-center gap-4 mt-1">
                  <div className="flex items-center gap-2"><span className="text-sm">Min:</span><input type="number" value={question.details.min || ''} onChange={e => handleDetailChange('min', e.target.value ? parseInt(e.target.value) : undefined)} className="w-24 p-1 border rounded-md text-sm" /></div>
                  <div className="flex items-center gap-2"><span className="text-sm">Max:</span><input type="number" value={question.details.max || ''} onChange={e => handleDetailChange('max', e.target.value ? parseInt(e.target.value) : undefined)} className="w-24 p-1 border rounded-md text-sm" /></div>
                </div>
              </div>
             )}
            <div>
