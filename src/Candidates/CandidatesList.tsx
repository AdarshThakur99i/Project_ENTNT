...
const CandidatesList: React.FC = () => {
  ...
  const rowVirtualizer = useVirtualizer({
    ...
    estimateSize: () => 80, // The height of each item in pixels
  });
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      {/* 3. The main container for the virtualized list */}
      <div ref={parentRef} className="border rounded-lg shadow-sm h-[70vh] bg-white overflow-auto">
        {isLoading ? (
            <div className="p-8 text-center text-gray-500">Loading candidates...</div>
        ) : searchedCandidates && searchedCandidates.length > 0 ? (
          // 4. A container for the total scrollable height
          <div style={{ height: `${rowVirtualizer.getTotalSize()}px`, width: '100%', position: 'relative' }}>
            {/* 5. Map over the virtual items provided by the hook */}
